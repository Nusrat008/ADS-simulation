...
def set_frequency_plan_and_common_options():
	...
	frequency_plan.startFrequency=2200000000
	frequency_plan.stopFrequency=2700000000
	frequency_plan.samplePointsLimit=50
	frequency_plan_list.append(frequency_plan)
	if 'minFreq' in empro.activeProject.parameters:
		empro.activeProject.parameters.setFormula('minFreq','2.2 GHz')
	if 'maxFreq' in empro.activeProject.parameters:
		empro.activeProject.parameters.setFormula('maxFreq','2.7 GHz')
...
class projImporter():
	...
	def create_materials(self,materialForEachLayer=False):
		...
		extraMaterialProperties=(162,1.778e-05,False,False) # (priority,thickness,modelTypeForMetals,convertedToResistance)
		material=ADSmaterialMap.get((ADSmaterialName,extraMaterialProperties),None)
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
			material=self.session.create_material(name=EMProMaterialName, color=(238,106,80,255), thickness="1.778e-05 m", resistance=0)
			try:
				material.details.electricProperties.parameters.thickness = "1.778e-05 m"
			except AttributeError:
				pass
		...
		ADSmaterialName=["Copper","__SubstrateLayer1"][materialForEachLayer]
		extraMaterialProperties=(50,None,None,False) # (priority,thickness,modelTypeForMetals,convertedToResistance)
		material=ADSmaterialMap.get((ADSmaterialName,extraMaterialProperties),None)
		if material == None:
			EMProMaterialName = self._getEMProMaterialName(ADSmaterialName,ADSmaterialMap,extraMaterialProperties,ADSmaterialsNo1to1)
			material=self.session.create_material(name=EMProMaterialName, color=(202,225,255,128), conductivity=58000000, imag_conductivity=0, permeability=1)
			try:
	...
	def create_geometry(self,topAssembly):
		...
		assembly=empro.geometry.Assembly()
		part=empro.geometry.Model()
		simBox = empro.geometry.Box( _createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox-xLowerBoundingBox", "abs((0.001-xLowerExtension)-(0.0302+xUpperExtension))"), _createIfToggleExtensionToBoundingBoxExpression("zUpperBoundingBox-zLowerBoundingBox", "((((stack_substrate1_layer_3_Z) + (zUpperExtension)) - (stack_substrate1_layer_1_Z)))"), _createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox-yLowerBoundingBox" , " abs((-0.0019-yLowerExtension)-(0.0273+yUpperExtension))"))
		part.recipe.append(simBox)
		part.coordinateSystem.anchorPoint = empro.geometry.CoordinateSystemPositionExpression(V(_createIfToggleExtensionToBoundingBoxExpression("(xUpperBoundingBox+xLowerBoundingBox)/2", "(0.0302+xUpperExtension+0.001-xLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("(yUpperBoundingBox+yLowerBoundingBox)/2", "(0.0273+yUpperExtension+-0.0019-yLowerExtension)/2"), _createIfToggleExtensionToBoundingBoxExpression("zLowerBoundingBox","((stack_substrate1_layer_1_Z) - (0))")))
		...
		self.session.adjust_view()
		assembly=empro.geometry.Assembly()
		pointString='0.0302+xUpperExtension#-0.0019-yLowerExtension;0.0302+xUpperExtension#0.0273+yUpperExtension;0.001-xLowerExtension#0.0273+yUpperExtension;0.001-xLowerExtension#-0.0019-yLowerExtension'
		sketch = self._create_sketch(pointString)
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex0",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","0.001-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.0019-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex1",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.0302+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.0019-yLowerExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex2",V(_createIfToggleExtensionToBoundingBoxExpression("xUpperBoundingBox","0.0302+xUpperExtension"),_createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox","0.0273+yUpperExtension"),0)))
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex3",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","0.001-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yUpperBoundingBox","0.0273+yUpperExtension"),0)))
		part=empro.geometry.Model()
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['0.0302#-0.0019;0.0302#0.0273;0.001#0.0273;0.001#-0.0019']
		...
		topAssembly.append(assembly)
		assembly=empro.geometry.Assembly()
		pointStrings=['0.033325#-0.005025;0.033325#0.030425;-0.002125#0.030425;-0.002125#-0.005025']
		sketch = None
		for pointString in pointStrings:
			sketch = self._create_sketch(pointString, sketch)
		sketch.constraintManager().append(empro.geometry.FixedPositionConstraint("vertex0",V(_createIfToggleExtensionToBoundingBoxExpression("xLowerBoundingBox","0.001-xLowerExtension"),_createIfToggleExtensionToBoundingBoxExpression("yLowerBoundingBox","-0.0019-yLowerExtension"),0)))
	...
	def create_ports( self, topAssembly, includeInvalidPorts=True, symbPinData=None):
		...
		head=V("(0.0156) - (0)","(-0.0019) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
		tail=V("(0.0156) - (0)","(-0.0019) - (0)","(stack_substrate1_layer_1_Z) - (0)")
		extent=empro.components.SheetExtent()
		extent.endPoint1Position=V("(0.001) - (0)","(-0.0019) - (0)","(stack_substrate1_layer_1_Z) - (0)")
		extent.endPoint2Position=V("(0.0302) - (0)","(-0.0019) - (0)","(((mask_cond_Zmax) + (mask_cond_Zmin)) / (2)) - (0)")
	...
	def create_parameters(self):
		...
		self._create_parameter("stack_substrate1_layer_3_Z", "1.6 mm", "Z of topology level (level 3 of stack substrate1)",True,fixGridAxis='Z')
		...
		self._create_parameter("mask_cond_Zmin",str("1.6 mm"),"Zmin of mask cond",True,fixGridAxis='Z')
